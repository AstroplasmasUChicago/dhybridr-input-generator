...
const SCHEMA = {
  ...
  ext_emf: {
    ...
    fields: [
      { key: 'Bx', label: 'Bx', type: 'str', dim: 0, default: '0.', hint: 'B_x function parser expression using x, y, z and ct(N). e.g. "1.", "cos(x)*ct(1)"' },
      { key: 'By', label: 'By', type: 'str', dim: 0, default: '0.', hint: 'B_y expression' },
      { key: 'Bz', label: 'Bz', type: 'str', dim: 0, default: '0.', hint: 'B_z expression' },
      { key: 'Ex', label: 'Ex', type: 'str', dim: 0, default: '0.', hint: 'E_x function parser expression using x, y, z and ct(N)' },
  ...
  species: {
    ...
    fields: [
      ...
      { key: 'vsp', label: 'vsp', type: 'strarr', dim: 'VDIM_STR',
        default: ['','',''], hint: 'Velocity expressions using x, y, z and ct(N). Overrides vdrift when set. Empty = use vdrift.',
      ...
      { key: 'nsp', label: 'nsp', type: 'str', dim: 0, default: '1.', hint: 'Density profile expression using x, y, z and ct(N) constants. Controls particle charge weight — use this for smooth density gradients. e.g. "0.5*(1-tanh((x-ct(1))/ct(2)))"' },
      { key: 'nsp_domain', label: 'nsp_domain', type: 'str', dim: 0, default: '', hint: 'Binary spatial mask: particles placed only where expression > 0. Does NOT scale density — use nsp for profiles. Empty = fill entire box. e.g. "if(sqrt((x-ct(5))^2+(y-ct(6))^2)<ct(8),1.,0.)"' },
  ...
  raw_diag: {
    ...
    fields: [
      ...
      { key: 'selectrule', label: 'selectrule', type: 'str', dim: 0, default: '1.', hint: 'Particle selection expression using x, y, z and ct(N). Dumps particle where result > 0.' },
